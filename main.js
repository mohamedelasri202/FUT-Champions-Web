    ...
    if (modifyBtn) {
      modifyBtn.addEventListener("click", (event) => {
        ...
        if (playerCard) {
          ...
          const nationalityURL = playerCard.querySelector(
            ".player-nationality"
          ).src;
          const clubURL = playerCard.querySelector(".player-club").src;
          const Position =
            playerCard.querySelector(".player-position").textContent;
          const pace = playerCard
            .querySelector(".player-pace")
            .textContent.split("\n")[1];
          const shooting = playerCard
            .querySelector(".player-shooting")
            .textContent.split("\n")[1];
          const passing = playerCard
            .querySelector(".player-passing")
            .textContent.split("\n")[1];
          const dribbling = playerCard
            .querySelector(".player-dribbling")
            .textContent.split("\n")[1];
          const defending = playerCard
            .querySelector(".player-defending")
            .textContent.split("\n")[1];
          const physical = playerCard
            .querySelector(".player-physical")
            .textContent.split("\n")[1];
  ...
  addPlayerBtn.addEventListener("click", (event) => {
    ...
    if (editingCard) {
      ...
      playerCard.className =
        "bg-[url('badge_total_rush.webp')] h-full w-full bg-contain bg-center bg-no-repeat flex flex-col items-center  player-card";
